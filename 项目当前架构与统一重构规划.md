...
`npm run check` 当前依次执行 `scripts/check.mjs`、`node --test tests/*.test.cjs`、`tests/regressions-v194.cjs`；包含架构护栏、模块行为和 jsdom 页面事件测试。V1.4.238 覆盖翻译关闭思维链、服务商 reasoning 隔离、缺失逗号恢复、原文/译文选区双向高亮和真实浏览器颜色烟测；最终数量见交付说明。它不是逐个启动真实 API、ComfyUI 或 Electron 的完整测试，也不能等同于独立完整 ESLint/全源码语法遍历。
