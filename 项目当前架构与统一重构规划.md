...
更新日期：2026-09-08。代码版本：**V1.4.237**。本文按当前源码核对，是后续开发的架构入口。

- 工作区：`F:\codex\ai-tag-characters`，分支 `feat/offline-characters-v198`。
- 桌面测试包：`C:\Users\admin\Desktop\AI绘画Tag工具箱V1.4.237`。
...
`translation-alignment.js` 解析模型返回的 JSON、代码块或普通文本，校验 ID 与片段拼接。只有所有片段逐字拼接等于完整译文且至少存在一处有效关联时，才返回 `alignment`；无效 ID 被过滤，整体映射不可用则仅保留 `text`。标点和补充语法可以用空 sourceIds。没有对照时不自动额外调用 AI 修复；该能力不会改变主 AI 对话调用的默认返回。

页面保留可编辑 textarea，在其背后绘制透明原文镜像，译文以纯文本片段展示。悬停临时高亮、点击固定高亮、选中文字双向高亮，Esc 清除。两侧仅沿直接关联定位，不通过共享片段继续扩大选区。原文滚动及窗口尺寸变化同步镜像；全部结果通过 textContent/text node 写入，模型输出不会作为 HTML 执行。复制按钮始终读取完整纯译文，包括原有空格与换行。
...
`npm run check` 当前依次执行 `scripts/check.mjs`、`node --test tests/*.test.cjs`、`tests/regressions-v194.cjs`；包含架构护栏、模块行为和 jsdom 页面事件测试。V1.4.237 本轮 **167 项通过**，覆盖 Tag 分段、长文降级、重复词/语序变化、多对多、异常数据降级、双向选区、直接关联边界、方向切换刷新参考、纯文本复制、HTML 安全与旧请求失效。它不是逐个启动真实 API、ComfyUI 或 Electron 的测试，也不能等同于独立完整 ESLint/全源码语法遍历。
...
本次交付说明：[V1.4.237](交付说明-V1.4.237.md)。提示词组接口详见 [Prompts README](src/modules/prompts.README.md)。
