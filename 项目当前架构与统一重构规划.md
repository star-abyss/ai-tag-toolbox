...
页面保留可编辑 textarea，在其背后绘制透明原文镜像，译文以纯文本片段展示。悬停临时高亮、点击固定高亮、选中文字双向高亮，Esc 清除。原文滚动及窗口尺寸变化同步镜像；全部结果通过 textContent/text node 写入，模型输出不会作为 HTML 执行。复制按钮始终读取完整纯译文，包括原有空格与换行。
...
`npm run check` 当前依次执行 `scripts/check.mjs`、`node --test tests/*.test.cjs`、`tests/regressions-v194.cjs`；包含架构护栏、模块行为和 jsdom 页面事件测试。V1.4.237 本轮 **165 项通过**，覆盖 Tag 分段、长文降级、重复词/语序变化、多对多、异常数据降级、双向选区、纯文本复制、HTML 安全与旧请求失效。它不是逐个启动真实 API、ComfyUI 或 Electron 的测试，也不能等同于独立完整 ESLint/全源码语法遍历。
