...
function createVisionService(options = {}) {
  ...
  const parseMetadata = typeof options.parseMetadata === 'function' ? options.parseMetadata : null;
  ...
  async function readMetadataMode(imageId, image) {
    ...
    try {
      if (!metadata) {
        // A fresh drop into the right-side vision slot has no pre-parsed
        // metadata (the temp record only keeps dataUrl/bytes). Parse the PNG
        // directly so built-in tags are available for any uploaded image,
        // not only image-store records created through add().
        if (typeof parseMetadata === 'function') {
          const source = image?.bytes || image?.dataUrl || (await imageDataUrl(image, imageId, {})).url;
          metadata = parseMetadata(source) || null;
        }
        if (!metadata && !visionTempStore && typeof images?.metadata === 'function') metadata = (await images.metadata(imageId)) || metadata;
      }
    } catch (error) {
