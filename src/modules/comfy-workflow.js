...
function analyzeWorkflow(value, options = {}) {
  const validation = validateApiWorkflow(value); if (!validation.ready) return { ready: false, level: 'manual', error: validation.error, nodeCount: 0, nodes: [], samplerCandidates: [], outputCandidates: [], promptCandidates: { positive: [], negative: [] }, fieldCandidates: {}, suggestedBindings: null, missingClasses: [], errors: [validation.error], warnings: [] };
  ...
  const fields = fieldCandidates(workflow, selected); const outputs = collectOutputCandidates(workflow, objectInfo); const ambiguous = samplers.length > 1 && !options.samplerId;
  const suggestedBindings = ambiguous ? null : { samplerId: selected?.nodeId || '', positive: positive.length === 1 ? positive : [], negative: negative.length <= 1 ? negative : [], ...Object.fromEntries(Object.entries(fields).map(([key, rows]) => [key, rows.length === 1 ? rows : []])), outputs: outputs.length === 1 ? [outputs[0].nodeId] : [] };
  ...
  return { ready: true, level: ambiguous || !selected || !positive.length || !outputs.length ? 'manual' : samplers.length === 1 ? 'standard' : 'advanced', nodeCount: entries.length, nodes: entries.map(([id]) => nodeRow(workflow, id)), samplerCandidates: samplers, outputCandidates: outputs, promptCandidates: { positive, negative }, fieldCandidates: fields, suggestedBindings, missingClasses, errors: [], warnings: ambiguous ? ['发现多个采样器，请选择主采样器'] : [] };
}
...
function validateBindings(workflowValue, bindings = {}, overrides = DEFAULT_OVERRIDES, options = {}) {
  ...
  for (const field of Object.keys(DEFAULT_OVERRIDES)) check(field, bindings[field]); return { ready: errors.length === 0, errors };
}
function applyExplicitBindings(workflowValue, bindings = {}, values = {}, overrides = DEFAULT_OVERRIDES, options = {}) {
  const validation = validateBindings(workflowValue, bindings, overrides, options); if (!validation.ready) throw new Error(validation.errors.join('；')); const workflow = parseWorkflow(workflowValue); const source = { ...values, negative: values.negative ?? '' };
  ...
  for (const field of Object.keys(DEFAULT_OVERRIDES)) set(field, source[field]); return workflow;
}
module.exports = { DEFAULT_OVERRIDES, parseWorkflow, validateApiWorkflow, analyzeWorkflow, collectOutputCandidates, validateBindings, applyExplicitBindings };
