...
async function runI18nTest(deps) {
  ...
  try {
    ...
    const ok = result && result.toEn && result.toZh && ids.includes('zh-CN') && ids.includes('en-US') && result.en.lang === 'en-US' && /AI Translate/.test(result.en.translate) && /Chinese/.test(result.en.direction) && /AI authentication failed/i.test(result.en.error) && /Back to AI chat/.test(result.en.prompt) && /Save as preset/.test(result.en.prompt) && /New extension library/.test(result.en.prompt) && /Collapse all/.test(result.en.prompt) && /Preview extension library/.test(result.en.prompt) && /Light mode/.test(result.en.theme) && /Dark mode/.test(result.en.theme) && /Follow system/.test(result.en.theme) && /Chat/.test(result.en.modes) && result.en.promptBody && result.en.helpTitle === '❓ Help' && /Extension Prompt Library/.test(result.en.helpBody) && /SillyTavern World Info/.test(result.en.helpBody) && /AI model:/.test(result.en.status) && /Extension prompt library:/.test(result.en.status) && /Upload, paste, or drop an image/.test(result.en.intro) && result.zh.lang === 'zh-CN' && /AI 翻译/.test(result.zh.translate) && /Tag/.test(result.zh.direction) && result.invalid && result.invalid.ok === false && result.invalid.code === 'LOCALE_INVALID_JSON';
