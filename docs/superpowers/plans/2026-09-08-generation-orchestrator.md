...
- [ ] **Step 2: Run the prompt and DOM tests and verify they fail because the sixth key and field are absent**

Run: `node --test tests/prompts-contract.test.cjs tests/prompts-composition.test.cjs tests/ui-dom.test.cjs`

- [ ] **Step 3: Implement V3 normalization, aliases, composition, prompt asset, editor field and six-item copy**
...
- [ ] **Step 1: Write failing tests for review, compare, invalid candidate IDs, JSON repair retry and image redaction**
...
- [ ] **Step 2: Run the focused tests and verify `SUBAGENT_UNAVAILABLE`**

Run: `node --test tests/candidate-evaluator.test.cjs tests/runtime-integration.test.cjs`

- [ ] **Step 3: Implement evaluator schemas, fixed protocol, authorized multi-image resolution and normalized outputs**
...
- [ ] **Step 4: Retry malformed provider JSON once with a repair instruction; return `OUTPUT_INVALID` after the second failure**
...
- [ ] **Step 1: Write failing tests for quick, auto, fixed3, retry, cancellation, needs-input and restart recovery**
...
- [ ] **Step 3: Implement normalized job/policy storage and deterministic phase events**
...
- [ ] **Step 4: Implement successful-render and attempt budgets, one retry per AI stage, score stopping, legal recommendation fallback and prompt patch application**
...
- [ ] **Step 5: Persist after every phase and turn restored `running` jobs into `interrupted` jobs**
...
- [ ] **Step 1: Write failing migration, binding, upload and render tests**
...
- [ ] **Step 2: Run ComfyUI tests and verify missing capability/upload behavior**

Run: `node --test tests/comfy-profiles.test.cjs tests/comfy-workflow.test.cjs tests/comfy-execution.test.cjs`

- [ ] **Step 3: Add profile migration, image-input candidates, explicit reference bindings and configuration UI selector**
...
- [ ] **Step 4: Implement authorized image byte upload and bind only the configured node; return `text_approximation` when no source binding exists**
...
- [ ] **Step 5: Ensure the monitor receives only a workflow hash, changed binding names and submitted parameters**
...
- [ ] **Step 1: Write failing tests for visible tools, one-round high-level execution, timeout extension, recoverable internal failures and successful-only Comfy count**
...
- [ ] **Step 2: Run integration tests and verify missing tools and old limiter behavior**

- [ ] **Step 3: Wire the orchestrator into Assistant and primary tools; expose compact schemas only**
...
- [ ] **Step 4: Change the primary prompt to use high-level generation tools and remove model-authored progress narration**

```text
绘图或复刻任务调用 generation.execute。不要自行循环调用 Tag、识图或 ComfyUI；程序会返回最终候选和实际提示词。
```

- [ ] **Step 5: Implement deadline extension and success-only Comfy counting without weakening ordinary tool limits**
...
- [ ] **Step 6: Add Assistant candidate selection methods and ensure the selected candidate controls final image and prompt fields**
...
- [ ] **Step 1: Write failing DOM tests for the three-way strategy control, auto-select, candidate scores, exact prompt copy, manual selection, stop and needs-input state**
...
- [ ] **Step 2: Run UI tests and verify controls/results are absent**

- [ ] **Step 3: Add compact controls beside ComfyUI status and persist settings**
...
- [ ] **Step 4: Connect generation events to the existing timeline and candidate cards; show final result and horizontal comparison without nested panels**
...
- [ ] **Step 5: Add localized labels and responsive constraints for 980px minimum and maximized widths**
...
- [ ] **Step 1: Add an end-to-end local fixture test covering create, recreate, two candidates, one revision, comparison, exact prompt, monitoring and cancellation**
...
- [ ] **Step 2: Run `npm run check`; require zero failures**

- [ ] **Step 3: Update all version markers and rerun `npm run check`**

```powershell
npm run check
```

- [ ] **Step 4: Move the previous exact-version desktop test directory to `F:\codex\desktop-backups`**
...
- [ ] **Step 5: Build and verify source copies, `app.asar`, native dependencies and executable**

```powershell
node work/finalize-desktop.cjs
```

- [ ] **Step 6: Launch the packaged Electron app against local fake AI/ComfyUI endpoints; verify 1100x800 and maximized layouts, 2-to-3 candidate flow, monitor export and source-image submission**

```powershell
node work/generation-smoke.cjs
```

- [ ] **Step 7: Commit final evidence without pushing or creating a release**
