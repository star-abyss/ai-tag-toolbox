...
`npm run check` 执行快速护栏、行为单元测试和 jsdom 页面测试。本轮 167 项通过；按项目规则不额外运行启动应用的慢测试。用户配置与图片保存在 `%APPDATA%\ai-tag-toolbox-rewrite`，桌面版本目录只是程序包。对照精度由所配置 AI 决定，长文本会增加返回量；本地翻译继续使用普通译文模式。
